...
@Component({
  selector: 'app-dailyClassUpdate-edit',
  templateUrl: './dailyClassUpdate-edit.component.html'
})
export class DailyClassUpdateEditComponent implements OnInit {
  ...
  onOptionItemClicked(key: string): void {
    if (key == "Create") {
      this.router.navigate(['/dailyClassUpdates/create', { id: -1 }]);
  ...
  Save(): void {
    ...
    this.dailyClassUpdate.Id = this.selectedId;
    this.dailyClassUpdate.RowVersionStr = this.objMaster.RowVersionStr;
    this.dailyClassUpdateService.update(this.dailyClassUpdate).subscribe({
      next: data => {
